...
inverte:
addi sp, sp, -24 # Armazenamos os valores de x12, x13 e x1 na pilha
sw x1, 0(sp)
sw x12, 16(sp)
sw x13 8(sp)
bge x13, x12, l1 # se a posição final for maior ou igual que a inicial, continuamos o procedimento
addi sp, sp, 24 # Desalocamos os valores inválidos que armazenamos na pilha
...
lw x13, 8(sp)
lw x12, 16(sp)
addi sp, sp, 24 # Desalocamos as três primeiras posições da pilha
